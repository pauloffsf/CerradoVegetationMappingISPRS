...
'''
Function to reassample the inferred patches in to a final infered image compatible with the original test image. It saves both the image and the MCLU (uncertainty)
...
It performs a deterministic test
'''
def GenTest():
	...
	metrics = MetricsCalc('./GenTest/GT_L1/', params.ASSEMBLE_OUT_DIR, './GenTest/S1/','./GenTest/S2/')
	metrics.StatsOnFile('stats.txt')
	print(metrics.confMatrix)
	metrics = MetricsGroupImg('./GenTest/GT_L1/', params.ASSEMBLE_OUT_DIR, './GenTest/S1/','./GenTest/S2/')
