...
'''
Codes above were obtained from: https://github.com/mlyg/unified-focal-loss/blob/main/loss_functions.py
'''
##################################
#       Dual Focal Loss          #
##################################

def dual_focal_loss (alpha = 1,beta = 1, gamma = 1, rho = 1): #por enquanto, valores semi-chutados. estudar melhor quais valores colocar aqui.
	'''
		Dual Focal Loss as described in the Paper: https://www.sciencedirect.com/science/article/abs/pii/S0925231221011310. It is a new loss function that unifies the Focal Loss with the Dual Cross Entropy for class
		imbalance in Semantic Segmentation Problems.
		alpha - alpha>=1
		beta - beta>=1
		gama - gama>=1
		rho - rho >=1selector
	...
	def loss_function(y_true, y_pred):
		...
		y_pred = K.clip(y_pred, epsilon, 1. - epsilon)
		cross_entropy = -y_true * K.log(y_pred)
		regularization = -beta*(1-y_true)*K.log(rho-y_pred)
		imbalance = alpha*K.pow(K.abs(y_true-y_pred), gamma)
		dual_focal_loss =  cross_entropy+regularization+imbalance
		dual_focal_loss = K.mean(K.sum(dual_focal_loss, axis=[-1]))
		return dual_focal_loss
...
def weighted_cross_entropy(weights = None):
	...
	def loss_function(y_true, y_pred):
		...
		weighted_cross_entropy=K.mean(K.sum(weighted_cross_entropy, axis=[-1]))
...
def loss_rq(pred, prior):

	logprior = K.log(prior+K.epsilon())
	r = K.softmax(logprior+K.log(K.softmax(pred,0)+K.epsilon()))
	loss_rq = K.sum(K.log(r)*r)-K.sum(K.log(pred+K.epsilon())*r)
	return K.mean(loss_rq)

def loss_qr(pred, prior):

	logprior = K.log(prior+K.epsilon())
	r = K.softmax(logprior+K.log(K.softmax(pred,0)+K.epsilon()))
	loss_qr = K.sum(K.log(pred+K.epsilon())*pred)-K.sum(K.log(r)*pred)
	return K.mean(loss_qr)


def combinedRQLoss():

	def loss_function(y_true, y_pred):
		pred, prior = tf.split(y_pred, num_or_size_splits=2, axis=-1)

		return loss_rq(pred, prior) + loss_qr(pred, prior)
...
##################################
# Calibrated Dual Focal Loss     #
##################################

def cal_dual_focal_loss (alpha = 1,beta = 1, gamma = 1, rho = 1):
	'''
		Dual Focal Loss as described in the Paper: https://www.sciencedirect.com/science/article/abs/pii/S0925231221011310.
		It is a new loss function that unifies the Focal Loss with the Dual Cross Entropy for class
		imbalance in Semantic Segmentation Problems.
		alpha - alpha>=1
		beta - beta>=1
		gama - gama>=1
	...
	def loss_function(y_true, y_pred):
		...
		# Clip values to prevent division by zero error
		epsilon = K.epsilon()
		rank = tf.sort(y_pred, axis=-1, direction='DESCENDING')
		rank = tf.unstack(rank, axis=-1)
		mclu=rank[0]-rank[1]
		cal_weight = 1-mclu
		y_pred = K.clip(y_pred, epsilon, 1. - epsilon)
		cross_entropy = -y_true * K.log(y_pred)
		cross_entropy=tf.transpose(cross_entropy, perm=[3, 0, 1, 2])
		cross_entropy = cal_weight*cross_entropy
		cross_entropy=tf.transpose(cross_entropy, perm=[1, 2, 3, 0])
		regularization = -beta*(1-y_true)*K.log(rho-y_pred)
		imbalance = alpha*K.pow(K.abs(y_true-y_pred), gamma)
		dual_focal_loss =  cross_entropy+regularization+imbalance
		dual_focal_loss = K.mean(K.sum(dual_focal_loss, axis=[-1]))
...
################################
#     Calibrated Focal loss    #
################################
def cal_focal_loss(alpha=None, beta=None, gamma_f=2.):
    """Focal loss is used to address the issue of the class imbalance problem. A modulation term applied to the Cross-Entropy loss function.
    Parameters
    ----------
    alpha : float, optional
    ...
    def loss_function(y_true, y_pred):
        ...
        if alpha is not None:
            cal = tf.transpose(cross_entropy, perm=[3, 0, 1, 2])
            cal = cal_weight*cal
            cal = tf.transpose(cal, perm=[1, 2, 3, 0])
            cal = alpha*cal
        else:
            cal = tf.transpose(cross_entropy, perm=[3, 0, 1, 2])
            cal = cal_weight*cal
